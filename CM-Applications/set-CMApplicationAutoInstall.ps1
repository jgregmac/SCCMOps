<#
Sets the AutoInstall property for all non-superseded, non-retired Applications to $true.
This configures the GUI option with the text "Allow this application to be installed from the Install Application task sequence without being deployed'.
Setting this option will enable the use of all applications in task sequences.
...
foreach ($app in $apps) {
    ...
    if ($appXML.AutoInstall -eq $false) {
        write-host $app.LocalizedDisplayName "is not configured for AutoInstall.  Fixing it..."
